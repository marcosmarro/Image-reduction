...
for i in range(len(reduced_science_files)):
    ...
    if i<121:
        tuple_array = centroid_sources(data, xpos=x_positions_1, ypos=y_positions_1, box_size=35, centroid_func=centroid_2dg)
        position = list(zip(tuple_array[0], tuple_array[1]))
        
    else:
        tuple_array = centroid_sources(data, xpos=x_positions_2, ypos=y_positions_2, box_size=35, centroid_func=centroid_2dg)
        position = list(zip(tuple_array[0], tuple_array[1]))
...
frequency, power = LombScargle(time, ratio/numpy.median(ratio)).autopower()
